  ...
  useEffect(() => {
    ...
    selectedFiles.forEach((data) => {
      ...
      if (type === "link") {
        ...
        const obj = {
          item: name,
          type: pathArray.length > 1 ? "folder" : "file",
        ...
        if (pathArray.length > 1) {
          const folderName = pathArray[0] == "" ? pathArray[1] : pathArray[0];
          ...
          if (!folderMap[folderName]) {
            ...
            newGeneratedArray.push(folderMap[folderName]);
          }
          folderMap[folderName].size += size;
          folderMap[folderName].status.push(obj);
        } else {
          newGeneratedArray.push(obj);
        }
      }
    });
    Object.keys(folderMap).forEach(function (key, index) {
      folderMap[key].size = formatBytes(folderMap[key].size);
    });

    setGeneratedArray(newGeneratedArray);
    setTableData(newGeneratedArray);
  }, [selectedFiles]);
